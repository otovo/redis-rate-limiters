...
master1="$( echo "$(redis-cli -u redis://127.0.0.1:6380 cluster nodes)" | sed -n 1p | awk '{print $1}')"
master2="$( echo "$(redis-cli -u redis://127.0.0.1:6380 cluster nodes)" | sed -n 2p | awk '{print $1}')"
master3="$( echo "$(redis-cli -u redis://127.0.0.1:6380 cluster nodes)" | sed -n 3p | awk '{print $1}')"
